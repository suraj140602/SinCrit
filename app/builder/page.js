...
function Home() {
  ...
  const handleImportDartCode = async () => {
    setIsParsing(true);
    try {
      const res = await fetch('/api/import-dart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dart_code: importCode })
      });
      ...
      if (data.success && data.json_tree) {
        ...
        const applyDefaultsToTree = (node) => {
          ...
          if (node.children) node.children = node.children.map(applyDefaultsToTree);
          return node;
        };
        ...
        commitHistory(newSchema);
        setSelectedId(parsedNode.id);

        if (data.unsupported_count > 0) {
          alert(`Import successful! ${data.unsupported_count} complex widgets were safely wrapped in CustomCode blocks.`);
        }
      ...
      alert("Error: Make sure your Python Parser Engine is running on port 8000!");
    } finally {
      setIsParsing(false);
    }
  };
    ...
    <div className="h-screen w-screen bg-[#0a0a0a] flex flex-col overflow-hidden relative font-sans text-gray-200 selection:bg-blue-500/30">
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-2xl bg-[#0E0F11] border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(16,185,129,0.15)] flex flex-col overflow-hidden">
              <div className="flex justify-between items-center p-6 border-b border-white/5 bg-[#161b22]">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-emerald-600/20 flex items-center justify-center border border-emerald-500/30">
                    <LucideIcons.Code2 size={20} className="text-emerald-400" />
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-white tracking-wide">Import Dart Code</h2>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest">AST Engine Auto-Translation</p>
                  </div>
                </div>
                <button onClick={() => setIsImportModalOpen(false)} className="text-gray-500 hover:text-white transition-colors bg-white/5 p-2 rounded-lg"><LucideIcons.X size={18} /></button>
              </div>
              ...
              <div className="p-6 bg-[#050505]">
                <textarea
                  ...
                  className="w-full h-64 bg-[#1A1B1E] text-emerald-400 font-mono text-xs p-4 rounded-xl border border-white/10 focus:border-emerald-500/50 outline-none resize-none custom-scrollbar shadow-inner"
                  placeholder="Paste your Flutter UI code here...&#10;e.g., Container(width: 100, child: Text('Hello'))"
                />
              </div>

              <div className="p-5 border-t border-white/5 bg-[#161b22] flex justify-end gap-3 shrink-0">
                <button onClick={() => setIsImportModalOpen(false)} className="px-5 py-2.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
                <button onClick={handleImportDartCode} disabled={isParsing || !importCode.trim()} className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-bold shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:bg-emerald-500 transition-colors flex items-center gap-2 disabled:opacity-50">
                  {isParsing ? <LucideIcons.Loader2 size={14} className="animate-spin" /> : <LucideIcons.Wand2 size={14} />}
                  {isParsing ? 'Parsing AST...' : 'Translate to Visual Blocks'}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      <div className="h-14 bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 shrink-0 relative z-[100] gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => setIsImportModalOpen(true)} className="hidden xl:flex px-3 py-1.5 bg-emerald-600/10 text-emerald-400 border border-emerald-600/20 text-[10px] font-bold rounded-xl hover:bg-emerald-600 hover:text-white transition-all items-center gap-1.5">
            <LucideIcons.Code2 size={12} /> Import Code
          </button>
